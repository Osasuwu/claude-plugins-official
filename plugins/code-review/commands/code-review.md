...
SYNCHRONOUS EXECUTION (binding, overrides any harness default): **every agent you launch in this command MUST be launched synchronously — `run_in_background: false` — and you MUST have all of its results in hand before you end your turn.** Never background an agent, never call `ScheduleWakeup`, never say you will "pick this up as each one reports in", and never end a turn while any agent is still pending. This command usually runs headless (`claude -p` in CI): the process exits at `end_turn`, so a backgrounded agent's completion notification is never delivered and its findings are lost. The failure is SILENT — the run reports `is_error=false` with zero denials and simply posts no comment, which the consuming repo's merge gate can only read as "review broke" (jarvis#1239, PR #1237: 12 reviewers backgrounded, `ScheduleWakeup(120s)` called, turn ended, nothing posted, three consecutive runs at ~$4.50 each). Posting the verdict comment is part of the SAME turn that launches the reviewers.
...
4. Then, launch 12 parallel reviewer agents (Sonnet for #1–6, #10, #11 — semantic review; Haiku for #7–9 and #12 — mechanical / pattern-based). "Parallel" here means launched together in ONE message and awaited in that same turn — `run_in_background: false` on every one of them, per the SYNCHRONOUS EXECUTION rule above. Backgrounding these twelve is the single most likely way to lose an entire review run. Prepend the following tool-discipline block, verbatim, to the prompt of EACH of the 12 reviewer agents **AND to EVERY other agent you launch anywhere in this command — the Haiku helpers of steps 1, 2, 3, 3.5, 5, and 7 included** (same delivery mechanism as the DESIGN-LOCKED block in step 3.5 — a preamble that is not prepended never reaches the subagent's context, and an un-armed helper agent that improvises a `find`/`grep`/piped command triggers exactly the denials this block exists to prevent):

   ```
   TOOL DISCIPLINE (binding, regardless of task pressure): your Bash usage is restricted to this command's allowed-tools allowlist — gh issue view/list, gh search, gh pr comment/diff/view/list, python -m py_compile, bash -n, node --check, git show, git blame, git log, wc. NEVER run `gh api`, `gh pr checks`, `curl`, `wget`, `git fetch`, `gh pr checkout`, or any other network/mutation/CI-status command — in headless CI these are DENIED, and every denial hard-fails the consuming repo's review gate even when the PR under review is clean. Do not fall back to WebFetch for GitHub data either. Note `gh pr checks` and `gh api` are BOTH un-allowlisted: a compound that self-notes "gh api not allowed, skip" and then falls through to `gh pr checks` is still DENIED as a whole — do not reach for either.

   EXACT-PREFIX MATCHING — the allowlist matches the literal command prefix, so a git invocation with a global flag before the subcommand does NOT match: `git -C <path> log`, `git -C <path> show`, `git --no-pager log` are all DENIED even though `git log` and `git show` are allowed. You are already running inside the repository checkout — call `git log`, `git show`, `git blame`, `git diff` with NO `-C` and no global flags. Likewise, never improvise around a missing capability: `find`, `ls`, `grep`, `sed`, `awk`, `jq`, `python3 -c`, `python -c`, and any shell redirection (`>`, `>>`, `|` into a writer) are NOT allowlisted — use the native Glob / Grep / Read tools instead, and use `python -m py_compile` / `bash -n` / `node --check` for syntax checks. A denial does not surface to you as a hard stop; it silently costs the consuming repo its review gate (jarvis#1239: five denials — `python3 -c`, `find`, and three `git -C` calls — failed an otherwise clean run that had already posted `No issues found.`).

   OUT OF SCOPE — merge readiness is NOT your concern. Your job is to review the diff and post a verdict; whether the PR is mergeable, up to date, or CI-green is the consuming workflow's decision, made from your posted verdict. NEVER inspect merge/CI state: no `gh pr checks`, no `gh api`, and do NOT request `mergeStateStatus`, `mergeable`, or CI/status-check fields via `gh pr view --json`. The only PR-state fields you ever need are `state`, `isDraft`, `headRefOid`, `commits`, `files`, `baseRefOid` (eligibility + diff scoping). Requesting merge-readiness fields is the exact over-reach that produced the denial on jarvis PR #1226 (`gh pr view --json ...mergeStateStatus,mergeable` and `gh pr checks`), and on a strict-tripwire consumer that single denial fail-closes an otherwise-clean review.

   CRITICAL — the allowlist matches ONE bare command only. A pipe, a redirect, a shell chain, or a directory-prefix flag breaks the glob match, so the WHOLE command is DENIED even though the base command is allowlisted. Every one of the following is DENIED and must NEVER be run: `git show X | wc -l`, `git show X > /tmp/f`, `git show X 2>/dev/null`, `git blame X | awk ...`, `git log ... | head`, `cmd1 && cmd2`, `cmd1; cmd2`, and any `git -C <path> ...` (never pass `-C` — you are already at the repo root). Run every allowlisted command **bare**: no pipe (`|`), no redirect (`>`, `>>`, `2>`), no chain (`&&`, `;`, `||`), no `-C` prefix. If you need to count, filter, or transform a command's output, do it in your own reasoning from the raw result — NEVER with a shell pipe.

   For reading and searching, do NOT use Bash: `grep`, `git grep`, `cat`, `find`, `ls`, `git ls-tree`, `head`, `sed`, `awk` are none of them allowlisted and will be DENIED. Use the native Grep and Glob tools to search the repo, and the native Read tool to read file contents; for file contents at a specific commit, bare `git show <sha>:<path>` is allowlisted. If data you want is unreachable with these tools, note the gap in your findings and move on — do NOT improvise around the allowlist.

   Never post-process the output of a prior tool call with a shell command. Do NOT `grep`, `cat`, `wc`, `head`, or otherwise shell-read the tool-result temp files under `~/.claude/` (e.g. `~/.claude/projects/.../tool-results/*.txt`) or any path outside the repo working tree — those commands are DENIED and, being outside the repo, are never useful to a reviewer. If a tool returned a large blob and you need to count or search within it (e.g. count backticks, find a substring), do that counting/searching in your own reasoning over the text already in your context — never by shelling out over a captured-output file.
   ```
   ...
   i. Agent #9 (structural growth tripwire, Haiku): For each non-deleted file in the PR, compute its post-PR line count and the delta vs base. Use `gh pr view <n> --json files,headRefOid,baseRefOid` to get the file list + head SHA + base SHA, then for each file run `git show <head_sha>:<file>` (post-PR) and `git show <base_sha>:<file>` (pre-PR) as **bare** commands and count the number of lines in each returned blob yourself. Do NOT pipe or redirect `git show` into `wc`/`awk`/`sed` or any other command, and do NOT append `2>/dev/null` — `git show <sha>:<path>` is allowlisted ONLY as a standalone command, so a pipe (`| wc -l`), a redirect (`> /tmp/f`, `2>/dev/null`), or any chaining makes the whole invocation un-allowlistable and it is DENIED in headless CI. A base-SHA path that doesn't exist yet (a new file) makes bare `git show` error — treat that as 0 lines for the pre-PR count. Flag any file that (a) crosses 1000 lines in this PR (was <1000, now ≥1000), or (b) grew by ≥300 lines in this PR and is now ≥800 lines. One line per finding: `<file>: <prev>→<new> lines (+<delta>)`. Skip lockfiles (`*.lock`, `package-lock.json`, `pnpm-lock.yaml`, `poetry.lock`, `uv.lock`, `Cargo.lock`), JSON/YAML fixtures and snapshots, generated files (under `generated/`, `build/`, `dist/`, `*.pb.go`, `*.gen.*`), vendored code, and minified bundles. No prose, no remediation advice — the file-size signal is purely mechanical; the human decides what to do.
