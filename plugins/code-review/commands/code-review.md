...
4. Then, launch 10 parallel reviewer agents (Sonnet for #1–6 and #10 — semantic review; Haiku for #7–9 — mechanical / pattern-based). The agents should do the following, then return a list of issues and the reason each issue was flagged (eg. CLAUDE.md adherence, bug, historical git context, diff coherence, cross-device, smoke, structural growth, simplification opportunity, etc.):
...
6. Filter out any issues with a score less than 80. Separate the remaining issues into two buckets:
   - **Code review bucket**: findings from agents #1–9.
